...
class ImagePreProcessService:
    def __init__(self):
        ...
        self.weightFile = "pose/coco/pose_iter_440000.caffemodel"
        self.nPoint = 18
    ...
    def getPreProcess(self, image):
        ...
        valid_pairs, invalid_pairs = self.getValidPairs(
            output, detected_keypoints, image.shape[1], image.shape[0]
        )
        personwiseKeypoints = self.getPersonwiseKeypoints(
            keypoints_list, valid_pairs, invalid_pairs
        )
        # 사람마다 검출한 스켈레톤 이어주기
        multiperson_skeleton = (
        ...
        multiperson_skeleton = cv2.resize(
            multiperson_skeleton, dsize=self.image_size, interpolation=cv2.INTER_AREA
        )
        return multiperson_skeleton


if __name__ == "__main__":
    multi = ImagePreProcessService()
    image = cv2.imread("dataset/result.png")
    frameClone = multi.getPreProcess(image=image)
